# Copilot CLI setup for macOS — not implemented yet

Planned entry point: `install-copilot-macos.sh` at the repo root.
...
| `setup-copilot-macos.sh` | Port `../windows/setup-copilot-windows.ps1`: same transactional stage → back up → swap → roll back shape, `brew` instead of `winget`, and the same two config roots (`--shared-source`, `--config-source`). |

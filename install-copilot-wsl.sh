...
# for WSL. Intended to be fetched and executed in one line:
#
...
#
#   curl -fsSL https://raw.githubusercontent.com/toperux/t4-ai-setup/main/install-copilot-wsl.sh | bash -s -- --skip-toolchain
#
...
# must not require it. A tarball rather than a zip because `tar` is present on
# every distro while `unzip` frequently is not - and it preserves the executable
# bit, which the zip does not.
#
...
INSTALLER="$WORKSPACE/repo/copilot/wsl/setup-copilot-wsl.sh"

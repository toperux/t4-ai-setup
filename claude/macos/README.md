# Claude Code setup for macOS — not implemented yet

Planned entry point: `install-claude-macos.sh` at the repo root.
...
| `setup-claude-macos.sh` | Port `../windows/setup-claude-windows.ps1`: same transactional stage → back up → swap → roll back shape, `brew` instead of `winget`, and the same two config roots (`--shared-source`, `--config-source`). |
